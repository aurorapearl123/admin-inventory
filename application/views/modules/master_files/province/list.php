					...
					<div class="card-head">
						...
						<div class="card-head-tools">
							<ul class="tools-list">
								...
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Print" onclick="popUp('<?php echo site_url('province/printlist') ?>', 800, 500)"><i class="la la-print"></i></button>
								</li>
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Export to Excel File" onclick="window.location='<?php echo site_url('province/exportlist') ?>';"><i class="la la-file-excel-o"></i></button>
								</li>
							</ul>
						</div>
					</div>
								...
								<table class="table table-striped has-img hover">
									<thead>
										<tr>
											<?php 
												$headers = array(
												    array('column_header'=>'Country','column_field'=>'countryID','width'=>'w-15','align'=>''),
												    array('column_header'=>'Province','column_field'=>'province','width'=>'w-15','align'=>''),
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<select class="form-control" id="countryID" name="countryID" data-live-search="true" liveSearchNormalize="true" style="width:100px">
													<option value="">&nbsp;</option>
													<?php 
													$recs = $this->db->get('countries')->result();
													foreach($recs as $rec){
													?>
													<option value="<?php echo $rec->countryID?>" <?php if($rec->countryID == $countryID){ echo "selected"; }?>><?php echo $rec->country?></option>
													<?php } ?>
												</select>
											</th>
											<th class="form-group form-input">
												<input type="text" class="form-control" id="province" name="province" style="width:200px" value="<?php echo $province ?>">
											</th>
											<th class="form-group form-input">
												<input type="text" class="form-control" id="remarks" name="remarks" style="width:200px" value="<?php echo $remarks ?>">
											</th>
											<th>
									...
									<tbody>
										<?php 
											if (count($records)) {
											    ...
											    ?>
										<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->provinceID); ?>'">
											...
											<td align="center">
												...
												if ($row->status == 1) {
													...
													echo "<span class='badge badge-pill badge-danger'>Dissolved</span>";
												}
												?>
											</td>
										</tr>

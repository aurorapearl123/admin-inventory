...
class Bank_accounts extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->_init_setup('Master Files', 'bank_accounts', 'bank_accounts', 'bankAccountID', 'bankAccountID');
		//change end
	}
    ...
    private function submenu()
    {
        ...
        //change start
        $this->data['current_module'] = $this->modules[$this->module]['sub']['Bank Accounts']; // defines the current sub module
        //change end
    ...
    private function _in_used($id = 0)
    {
        //change start
        $tables = array(
            'bank_deposits' => 'bankAccountID',
            'payment_headers' => 'bankAccountID'
        );
        //change end
    ...
    public function save()
    {
        ...
        //change start
        $table_fields = array('bankID','ownerID', 'accountName', 'accountNo','accountType','bankAccountType','email');
        //change end
        ...
        if ($this->roles['create']) {
            ...
            if ($this->record->save()) {
                ...
                $logs = "Insert - " .trim($this->input->post('accountName'));
    ...
    {   
        ...
        if ($this->roles['edit']) {
            ...
            $this->record->fields[] = 'banks.bankName';
		
			// set joins
            $this->record->joins[]  = array('banks',$this->table.'.bankID=banks.bankID','left');
    ...
    public function update()
    {
        ...
        //change start
        $table_fields = array('bankID','ownerID', 'accountName', 'accountNo','accountType','bankAccountType','email','status');
        //change end
    ...
    public function view($id)
    {
        ...
        if ($this->roles['view']) {
			...
			// extend fields - join tables			
		      $this->record->fields[] = 'banks.bankName';
              $this->record->fields[] = 'owners.firstName';
              $this->record->fields[] = 'owners.lastName';
              $this->record->fields[] = 'owners.middleName';
        
            // set joins
            $this->record->joins[]  = array('banks',$this->table.'.bankID=banks.bankID','left');
            $this->record->joins[]  = array('owners',$this->table.'.ownerID=owners.ownerID','left');
    ...
    public function show()
    {
        ...
        $condition_fields = array(
			array ('variable' => 'bankName', 'field' => 'banks.bankName', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'lastName', 'field' => 'owners.lastName', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'accountName', 'field' => $this->table . '.accountName', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'accountNo', 'field' => $this->table . '.accountNo', 'default_value' => '', 'operator' => 'where' ), 
            array ('variable' => 'accountType', 'field' => $this->table . '.accountType', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'bankAccountType', 'field' => $this->table . '.bankAccountType', 'default_value' => '', 'operator' => 'like_both' ), 
		
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '1', 'operator' => 'where' ) 
        );
        
        // sorting fields
        $sorting_fields = array('accountName'=>'asc');
        //change end
        //=========================
    	...
    	// select
    		...
    		$this->db->select('banks.bankName');
            $this->db->select('owners.lastName');
            $this->db->select('owners.firstName');
            $this->db->select('owners.middleName');
    
    	// from
    		$this->db->from($this->table);
    	    	
    	// join   	
    	   $this->db->join('banks',$this->table.'.bankID=banks.bankID', 'left');
           $this->db->join('owners',$this->table.'.ownerID=owners.ownerID', 'left');
    ...
    public function printlist()
    {
        ...
        $data['title'] = ucwords($this->module_label)." List";
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                    ...
                    if ($row->accountType == 1) {
                        $accountType= "Checking Account (PHP)";
                    } else if ($row->accountType == 2) {
                        $accountType= "Savings Account (PHP)";
                    } else if ($row->accountType == 3) {
                        $accountType= "Savings Account (PHP)";
                    } else if ($row->accountType == 4) {
                        $accountType= "Checking Account (USD)";
                    }
                     if ($row->bankAccountType == "Income") {
                        $bankAccountType= "Income";
                    } else if ($row->bankAccountType == "Expense") {
                        $bankAccountType= "Expense";
                    } 

                                       
                $data .= "<Row>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $ctr . ".</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->bankName . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->lastName.', '.$row->firstName.' '.$row->middleName  . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->accountName . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->accountNo . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $accountType . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $bankAccountType . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->email . "</Data></Cell>";
                if ($row->status == 1) {
    ...
    {
        //change start
        $this->db->where('accountName', trim($this->input->post('accountName')));
        $this->db->where('accountNo', trim($this->input->post('accountNo')));
        //change end

...
class Item_canvas extends CI_Controller {
	//Default Variables
	var $menu;
	var $roles;
	var $data;
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('item_canvas'); // defines contoller link
		$this->table = 'itempricecanvas'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'canvasID'; // defines primary key
		$this->logfield = 'canvasID';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Item_canvas'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Item Canvas']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('itemID', 'suppID', 'price','minorderqty', 'umsr');
		...
		if ($this->roles['create']) {
			...
			$this->records->fields['priceExpiry'] = date('Y-m-d',strtotime(trim($this->input->post('priceExpiry'))));
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('itemID', 'suppID', 'price','minorderqty', 'umsr','status');
	...
	public function show() {
		...
		$condition_fields = array (
			...
			array ('variable' => 'suppID', 'field' => $this->table . '.suppID', 'default_value' => '', 'operator' => 'where' ), 
			array ('variable' => 'price', 'field' => $this->table . '.price', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'minorderqty', 'field' => $this->table . '.minorderqty', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'umsr', 'field' => $this->table . '.umsr', 'default_value' => '', 'operator' => 'brandID' ), 
			array ('variable' => 'priceExpiry', 'field' => $this->table . '.priceExpiry', 'default_value' => '', 'operator' => 'brandID' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('priceExpiry' => 'desc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->price."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->minorderqty."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->umsr."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->priceExpiry."</Data></Cell>";
				if ($row->status == 1) {

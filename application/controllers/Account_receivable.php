...
class Account_receivable extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->_init_setup('Transactions', 'account_receivable', 'arheaders', 'arID', 'invoiceNo');
		//change end
	}
    ...
    private function submenu()
    {
        ...
        //change start
        $this->data['current_module'] = $this->modules[$this->module]['sub']['Account Receivable']; // defines the current sub module
        //change end
    ...
    public function create()
    {
        ...
        if ($this->roles['create']) {
            ...
            $this->frameworkhelper->clear_session_item('ardetails');
    ...
    public function save()
    {
        ...
        //change start
        $table_fields = array('invoiceNo','custID','discount','paymentTerm','amountPaid','balance','remarks');
        //change end
        

        // arID    soNo    invoiceDate  suppID  grossAmount discount    netAmount   paymentTerm    dueDate amountPaid  balance datePaid    createdBy   dateCreated confirmedBy dateConfirmed   cancelledBy dateCancelled   dateInserted    dateLastEdit    dateDeleted deletedBy   remarks status 1: pending; 2: confirm; 3: partial delivery; 4: full
        ...
        if ($this->roles['create']) {
            ...
            if ($this->record->save()) {
                ...
                // id  arID    productID   qty price   amount  delQty  delStatus   dateInserted    dateLastEdit    dateDeleted deletedBy   status 1: pending; 2: confirm; 3: partial delivery; 4: full
                $total = 0;
                if (!empty($_SESSION['ardetails'])) {
                    $batch = array();
                    foreach($_SESSION['ardetails'] as $product){
                        $info = array();
                        $info['arID']           = $this->record->field->arID;
                        $info['drID']         = $product['drID'];                       
                        $info['soID']            = trim($this->input->post('soID'));
                    ...
                    $this->db->insert_batch('ardetails', $batch);
                }
                //change end
    ...
    public function update()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $id = $this->encrypter->decode($this->input->post('arID'));
        //change start
        $table_fields = array('invoiceNo','custID','discount','paymentTerm','amountPaid','balance','remarks');
        //change end
        // check roles
    ...
    public function view($id)
    {
        ...
        if ($this->roles['view']) {
            ...
            $this->db->select('ardetails.arID');
            $this->db->select('ardetails.amount');
            $this->db->select('ardetails.amountPaid');
            ...
            $this->db->where('ardetails.arID', $data['rec']->arID);
            $this->db->where('ardetails.status >', 0);
    ...
    public function display_session_products($display_area='')
    {
        ...
        $headers = array(
            ...
            array('column_header'=>'SO No','column_field'=>'','width'=>'w-10','text-align'=>'text-center','align'=>'left'),
            array('column_header'=>'DR No','column_field'=>'','width'=>'w-10','text-align'=>'text-center','align'=>'left'),
        ...
        $display = array(
            ...
            // array('align'=>'left','field'=>'drID'),
            array('align'=>'Center','field'=>'soNo'),
            array('align'=>'center','field'=>'drNo'),
            array('align'=>'center','field'=>'deliveryDate'),
    ...
    public function print_record($id=0)
    {
        ...
        // for retrieve with joining tables -------------------------------------------------
            ...
            $this->db->where($this->pfield, $id);
            // ----------------------------------------------------------------------------------
            $data['arheaders'] = $this->db->get()->row();
            
            // get details
            $this->db->select('ardetails.arID');
            $this->db->select('ardetails.qty');
            $this->db->select('ardetails.price');
            $this->db->select('ardetails.amount');  
            $this->db->select('ardetails.productID');        
            $this->db->select('products.name');
            $this->db->select('products.description');
            $this->db->select('products.umsr');
            $this->db->select('products.productCode');
            ...
            $this->db->from('ardetails');
            $this->db->join('products', 'ardetails.productID=products.productID','left');
            $this->db->join('category', 'products.catID=category.catID','left');

            $this->db->where('ardetails.arID', $data['arheaders']->arID);
            $this->db->where('ardetails.status !=',-100);

            $this->db->order_by('products.productID','ASC');
            $data['ardetails'] = $this->db->get()->result();
            ...
            $this->mpdf->Output("account_receivable_".$data['arheaders']->invoiceNo.".pdf","I");

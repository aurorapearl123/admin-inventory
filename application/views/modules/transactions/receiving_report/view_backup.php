...
<script>
	...
	function confirm_record(status='') {
		...
		var poID = "<?php echo $headers->poID; ?>";
		...
		if (status == 4) { //close override	
			...
			.then((closeReason) => {
				if (closeReason.value) {
					console.log('closeReason',closeReason);
						$.post("<?php echo site_url()?>purchase_order/close_record", { status: status, poID: poID, closeReason: closeReason.value},
							function(response){ 							
								window.location='';
							}, "json");
				}
			...
			.then((willDelete) => {
				if (willDelete.value) {
						$.post("<?php echo site_url()?>purchase_order/confirm_record", { status: status, poID: poID},
							function(response){ 							
								window.location='';
							}, "json");
				}

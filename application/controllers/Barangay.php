...
class Barangay extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('barangay'); // defines contoller link
		$this->table = 'barangays'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'barangayID'; // defines primary key
		$this->logfield = 'barangay';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Barangay'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Barangay']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('provinceID','cityID', 'barangay', 'remarks');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['barangayID'] = $this->db->insert_id();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('provinceID','cityID', 'barangay', 'remarks', 'status');
	...
	public function show() {
		...
		$condition_fields = array (
			...
			array ('variable' => 'cityID', 'field' => $this->table . '.cityID', 'default_value' => '', 'operator' => 'where' ), 
			array ('variable' => 'barangay', 'field' => $this->table . '.barangay', 'default_value' => '', 'operator' => 'like_both' ), 
		...
		// sorting fields
		$sorting_fields = array ('province' => 'asc','city' => 'asc','barangay' => 'asc' );
	...
	public function check_duplicate() {
		$this->db->where('barangay', trim($this->input->post('barangay')));

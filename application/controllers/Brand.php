...
class Brand extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('brand'); // defines contoller link
		$this->table = 'brands'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'brandID'; // defines primary key
		$this->logfield = 'brand';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Brand'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Brand']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function getBrand()
	{
	    ...
	    $records = $this->db->get('brands');
	    echo $this->frameworkhelper->get_json_data($records, 'brandID', 'brand');
	}
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('brand', 'description');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['brandID'] = $this->db->insert_id();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('brand', 'description', 'status');
	...
	public function check_duplicate() {
		$this->db->where('brand', trim($this->input->post('brand')));

					...
					<form method="post" name="frmEntry" id="frmEntry" action="<?php echo $controller_page ?>/save" enctype="multipart/form-data">
						...
						<div class="table-row">
							<table class="table-form column-3">
								<tbody class="">
									<tr>
										<td class="form-label" width="12%">SO No:<span class="asterisk">*</span></td>
										<td class="form-group form-input" width="21.33%" nowrap>

											<select class="form-control " id="soID" name="soID" data-live-search="true" livesearchnormalize="true" title="Customer"  onchange="get_dr_collectibles();" required>
												<option value="" selected>&nbsp;</option>
												<?php 
												$this->db->where('status', 4);
												$results = $this->db->get('soheaders')->result();
												foreach($results as $res) { ?>
													<option value="<?php echo $res->soID ?>"><?php echo $res->soNo ?></option>
												<?php } ?>
											</select>
										</td>
								...
								<table id="table_products" class="table hover">
									<thead class="thead-light" align="center">
										<tr>
											...
											<th class="w-10"><div align="center">SO No</div></th>
...
function get_dr_collectibles()
{
	var soID = $('#soID').val();
	// console.log(drID);

	$.post(site_url+'generic_ajax/get_dr_collectibles/', { soID: soID },
		function(data){
			console.log(data);

			tm_display_session_products('ardetails', 'div_products');


			$('#custID').val(data.rec.custID);
			...
			$('#custID').selectpicker('val', data.rec.custID);
			$('#paymentTerm,#custID').selectpicker('render');
			$('#paymentTerm,#custID').selectpicker('refresh');

					...
					<div class="card-head">
						...
						<div class="card-head-tools">
							<ul class="tools-list">
								...
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Print" onclick="popUp('<?php echo site_url('barangay/printlist') ?>', 800, 500)"><i class="la la-print"></i></button>
								</li>
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Export to Excel File" onclick="window.location='<?php echo site_url('barangay/exportlist') ?>';"><i class="la la-file-excel-o"></i></button>
								</li>
							</ul>
						</div>
					</div>
								...
								<table class="table table-striped has-img hover">
									<thead>
										<tr>
											<?php 
												$headers = array(
												    array('column_header'=>'Province','column_field'=>'province','width'=>'w-10','align'=>''),
												    array('column_header'=>'City','column_field'=>'city','width'=>'w-15','align'=>''),
												    array('column_header'=>'Barangay','column_field'=>'barangay','width'=>'w-15','align'=>''),
												    array('column_header'=>'Remarks','column_field'=>'remarks','width'=>'w-15','align'=>''),
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<select class="form-control" id="provinceID" name="provinceID" data-live-search="true" liveSearchNormalize="true" onchange="getCities();">
													<option value="">&nbsp;</option>
													<?php 
													$recs = $this->db->get('provinces')->result();
													foreach($recs as $rec){
													?>
													<option value="<?php echo $rec->provinceID?>" <?php if($rec->provinceID == $provinceID){ echo "selected"; }?>><?php echo $rec->province?></option>
													<?php } ?>
													
												</select>
											</th>
											<th class="form-group form-input">
												<select class="form-control" id="cityID" name="cityID" data-live-search="true" liveSearchNormalize="true">
													<option value="">&nbsp;</option>
													<?php 
													$recs = $this->db->get('cities')->result();
													foreach($recs as $rec){
													?>
													<option value="<?php echo $rec->cityID?>" <?php if($rec->cityID == $cityID){ echo "selected"; }?>><?php echo $rec->city?></option>
													<?php } ?>
													
												</select>
											</th>
											<th class="form-group form-input">
												<input type="text" class="form-control" id="barangay" name="barangay" style="width:150px" value="<?php echo $barangay ?>">
											</th>
											<th class="form-group form-input">
									...
									<tbody>
										<?php 
											if (count($records)) {
											    ...
											    ?>
										<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->barangayID); ?>'">

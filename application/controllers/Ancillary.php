...
class Ancillary extends CI_Controller {
	//Default Variables
	var $menu;
	var $roles;
	var $data;
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('ancillary'); // defines contoller link
		$this->table = 'ancillaries'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'ancillaryID'; // defines primary key
		$this->logfield = 'barangay';
        $this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/ancillary'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Ancillary']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	private function _in_used($id = 0) {
		$tables = array('ancillaries' => 'ancillaryID');
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
        $table_fields = array ('ancillaryCode','division','officeName', 'location', 'officehead');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['ancillaryID'] = $this->db->insert_id();

				$this->db->select('itemID');		
				$items = $this->db->get('items')->result();	

				for($ancillaryID = 1; $ancillaryID <= 3; $ancillaryID++) {
					echo 'ancillaryID = ' .$ancillaryID . '<br/>';
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('ancillaryCode','division','officeName', 'location', 'officehead', 'status');
	...
	public function show() {
		...
		$condition_fields = array (
			array ('variable' => 'division', 'field' => $this->table . '.division', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'officeName', 'field' => $this->table . '.officeName', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'location', 'field' => $this->table . '.location', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'officehead', 'field' => $this->table . '.officehead', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('division' => 'asc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->location."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->officehead."</Data></Cell>";
				if ($row->status == 1) {
	...
	public function check_duplicate() {
        ...
        $officeName =  trim($this->input->post('officeName'));
        $location =  trim($this->input->post('location'));
        //$officeHead =  trim($this->input->post('officeHead'));

        $this->db->where('division',$division);
        $this->db->where('officeName',$officeName);
        $this->db->where('location',$location);
       // $this->db->where('division',$officeHead);

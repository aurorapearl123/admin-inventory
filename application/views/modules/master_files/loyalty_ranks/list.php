					...
					<div class="card-head">
						...
						<div class="card-head-tools">
							<ul class="tools-list">
								...
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Print" onclick="popUp('<?php echo site_url('loyalty_ranks/printlist') ?>', 800, 500)"><i class="la la-print"></i></button>
								</li>
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Export to Excel File" onclick="window.location='<?php echo site_url('loyalty_ranks/exportlist') ?>';"><i class="la la-file-excel-o"></i></button>
								</li>
							</ul>
						</div>
					</div>
								...
								<table class="table table-striped has-img hover">
									<thead>
										<tr>
											<?php 
												$headers = array(
												    array('column_header'=>'Rank Name','column_field'=>'rankName','width'=>'w-15','align'=>''),
												    array('column_header'=>'Points Required','column_field'=>'pointsRequired','width'=>'w-15','align'=>''),
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<input type="text" class="form-control" id="rankName" name="rankName"  value="<?php echo $rankName ?>">
											</th>
											<th class="form-group form-input">
												<input type="text" class="form-control" id="pointsRequired" name="pointsRequired"  value="<?php echo $pointsRequired ?>">
											</th>											
											<th>
									...
									<tbody>
										<?php 
											if (count($records)) {
											    ...
											    ?>
											<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->rankID); ?>'">
											<td><?php echo $row->rankName ?></td>
											<td align="left"><?php echo $row->pointsRequired ?></td>										
											<td align="center">
												<?php 
												if ($row->status == 1) {

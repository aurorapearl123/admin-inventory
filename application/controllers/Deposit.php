...
class Deposit extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('deposit'); // defines contoller link
		$this->table = 'bank_deposits'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'depositID'; // defines primary key
		$this->logfield = 'deposit';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Deposit'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Deposit']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('bankAccountID', 'amount', 'remarks');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['depositID'] = $this->db->insert_id();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('bankAccountID','amount', 'remarks', 'status');
	...
	public function view($id) {
		...
		if ($this->roles['view']) {
			...
			$this->db->select ( 'bank_accounts.email' );
			
		
			// select
			$this->db->select ( $this->table . '.amount' );
			$this->db->select ( $this->table . '.depositID' );
	...
	public function show() {
		...
		$condition_fields = array (
			array ('variable' => 'bankAccountID', 'field' => $this->table . '.bankAccountID', 'default_value' => '', 'operator' => 'where' ), 
			array ('variable' => 'email', 'field' => 'bank_accounts.email', 'default_value' => '', 'operator' => 'like_both' ), 
		...
		// sorting fields
		$sorting_fields = array ('bankAccountID' => 'asc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s26'><Data ss:Type='String'>".$row->amount."</Data></Cell>";
	...
	public function check_duplicate() {
		// $this->db->where('province', trim($this->input->post('province')));

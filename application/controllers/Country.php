...
class Country extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('country'); // defines contoller link
		...
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Country'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Country']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save_popup() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('countryCode','country', 'nationality','remarks');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['countryID'] = $this->db->insert_id();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('countryCode','country', 'nationality','remarks','status');
	...
	public function show() {
		...
		$condition_fields = array (
			array ('variable' => 'countryCode', 'field' => $this->table . '.countryCode', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'country', 'field' => $this->table . '.country', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'nationality', 'field' => $this->table . '.nationality', 'default_value' => '', 'operator' => 'like_both' ), 
		...
		// sorting fields
		$sorting_fields = array ('country' => 'asc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->countryCode."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->country."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->nationality."</Data></Cell>";
	...
	public function check_duplicate() {
		$this->db->where('country', trim($this->input->post('country')));

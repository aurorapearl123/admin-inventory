...
class Shift extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('shift'); // defines contoller link
		$this->table = 'shift'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'shiftID'; // defines primary key
		$this->logfield = 'shift';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Shift'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Shift']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	private function _in_used($id = 0) {
		$tables = array('cities' => 'provinceID');
	...
	public function save() {
		...
		$table_fields = array ('startTime', 'endTime');
		...
		if ($this->roles['create']) {
			...
			foreach ($table_fields as $fld) {
				...
				$this->records->fields[$fld] = date('Y-m-d G:i',strtotime($this->input->post($fld)));
			}
	...
	public function save_popup() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('countryID', 'province', 'remarks');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['provinceID'] = $this->db->insert_id();
				$this->records->retrieve();
				// record logs
				$logs = "Record - ".trim($this->input->post($this->logfield));
				$this->log_model->table_logs($data['current_module']['module_label'], $this->table, $this->pfield, $this->records->field->$data['pfield'], 'Insert', $logs );
	...
	public function edit($id) {
		...
		if ($this->roles['edit']) {
			...
			$this->records->retrieve();
			// ----------------------------------------------------------------------------------
            $data['records'] = $this->records->field;
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('startTime','endTime', 'status');
		...
		if ($this->roles['edit']) {
			...
			foreach ($table_fields as $fld) {
                if($fld == 'status') {
					...
					$this->records->fields[$fld] = date('Y-m-d G:i',strtotime($this->input->post($fld)));
                    //$this->records->fields[$fld] = date('Y-m-d G:i',mktime(date("G:i", strtotime($this->input->post($fld)))));
                }
	...
	public function view($id) {
		...
		if ($this->roles['view']) {
			// for retrieve with joining tables -------------------------------------------------
			//$this->db->select ( 'accounts.accountName' );
	...
	public function show() {
		...
		$condition_fields = array (
            ...
            array ('variable' => 'startTime', 'field' => $this->table . '.startTime', 'default_value' => '', 'operator' => 'like_both' ),
            array ('variable' => 'endTime', 'field' => $this->table . '.endTime', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('shiftID' => 'asc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".date('h:i A', strtotime($row->startTime))."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".date('h:i A', strtotime($row->endTime))."</Data></Cell>";
	...
	public function check_duplicate() {
		...
		$startTime = date('Y-m-d G:i',strtotime($this->input->post('startTime')));

		$endTime = date('Y-m-d G:i',strtotime($this->input->post('endTime')));

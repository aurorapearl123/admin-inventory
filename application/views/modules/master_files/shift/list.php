					...
					<div class="card-head">
						...
						<div class="card-head-tools">
							<ul class="tools-list">
								...
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Print" onclick="popUp('<?php echo site_url('shift/printlist') ?>', 800, 500)"><i class="la la-print"></i></button>
								</li>
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Export to Excel File" onclick="window.location='<?php echo site_url('shift/exportlist') ?>';"><i class="la la-file-excel-o"></i></button>
								</li>
							</ul>
						</div>
					</div>
								...
								<table class="table table-striped has-img hover">
									<thead>
										<tr>
											<?php 
												$headers = array(
												    array('column_header'=>'Start Time','column_field'=>'startTime','width'=>'w-15','align'=>''),
												    array('column_header'=>'End Time','column_field'=>'endTime','width'=>'w-15','align'=>''),
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<?php  $startTime = ($startTime != NULL) ? strtotime($startTime) : "--:-- --"?>
												<input type="time" id="startTime" class="form-control" name="startTime"  value="<?php echo date('h:i:s', $startTime)?>" >
											</th>

											<th class="form-group form-input">
												<?php  $endTime = ($endTime != NULL) ? strtotime($endTime) : "--:-- --"?>
												<input type="time" id="endTime" class="form-control" name="endTime" value="<?php echo date('h:i:s', $endTime)?>" >
											</th>
									...
									<tbody>
										<?php 
											if (count($records)) {
											    ...
											    ?>
										<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->shiftID); ?>'">

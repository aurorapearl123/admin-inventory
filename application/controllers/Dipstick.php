 ...
 */
class Dipstick extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->_init_setup('Master Files', 'Dipstick', 'dipstick_inventory', 'dipID', 'inTime');
		//change end
	}
    ...
    private function submenu()
    {
        ...
        //change start
        $this->data['current_module'] = $this->modules[$this->module]['sub']['Dipstick']; // defines the current sub module
        //change end
    ...
    public function save()
    {
        ...
        //change start
        $table_fields = array('shiftID', 'dipper','openingQty','closingQty','varianceQty','openingMsr', 'closingMsr', 'varianceMsr');
        //change end
        ...
        if ($this->roles['create']) {
            ...
            $this->record->fields['inTime'] = date('Y-m-d G:i',strtotime($this->input->post('inTime')));
            $this->record->fields['outTime'] = date('Y-m-d G:i',strtotime($this->input->post('outTime')));
    ...
    {   
        ...
        if ($this->roles['edit']) {
            ...
            // set table
            $this->db->select('shift.startTime');
            $this->db->select('shift.endTime');
            $this->db->select ( $this->table . '.*' );
		
			// from
            $this->db->from ( $this->table );
            $this->db->join ( 'shift', $this->table . '.shiftID=shift.shiftID', 'left' );
    ...
    public function update()
    {
        ...
        //change start
        $table_fields = array('shiftID', 'dipper', 'inTime','outTime','openingQty','closingQty','varianceQty','openingMsr', 'closingMsr', 'varianceMsr', 'status');
        //change end
        ...
        if ($this->roles['edit']) {
            ...
            foreach ($table_fields as $fld) {
                if($fld == "inTime") {
                    $this->record->fields[$fld] = date('Y-m-d G:i', strtotime($this->input->post($fld)));
                }
                else if($fld == "outTime") {
                    $this->record->fields[$fld] = date('Y-m-d G:i', strtotime($this->input->post($fld)));
                }
    ...
    public function show()
    {
        ...
        $condition_fields = array(
			array ('variable' => 'inTime', 'field' => $this->table . '.inTime', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'outTime', 'field' => $this->table . '.outTime', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'contactperson', 'field' => $this->table . '.contactperson', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'email', 'field' => $this->table . '.email', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) 
        );
        
        // sorting fields
        $sorting_fields = array('dipID'=>'asc');
        //change end
        //=========================
    ...
    public function printlist() {
        ...
        // array('column_header'=>'Time In','column_field'=>'inTime','width'=>'w-15','align'=>''),
        // array('column_header'=>'Time Out','column_field'=>'outTime','width'=>'w-15','align'=>''),
        // array('column_header'=>'Opening Qty','column_field'=>'openingQty','width'=>'w-15','align'=>''),
        // array('column_header'=>'Closing Qty','column_field'=>'closingQty','width'=>'w-15','align'=>''),
        // array('column_header'=>'Variance Qty','column_field'=>'varianceQty','width'=>'w-15','align'=>''),
        // array('column_header'=>'Opening Msr','column_field'=>'openingMsr','width'=>'w-15','align'=>''),
        // array('column_header'=>'Closing Msr','column_field'=>'closingMsr','width'=>'w-15','align'=>''),
        // array('column_header'=>'Variance Msr','column_field'=>'varianceMsr','width'=>'w-15','align'=>''),
    ...
    public function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . date('h:i A', strtotime($row->inTime)) . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . date('h:i A', strtotime($row->outTime)) . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->openingQty . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->closingQty . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->varianceQty . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->openingMsr . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->closingMsr . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->varianceMsr . "</Data></Cell>";
                if ($row->status == 1) {
    ...
    {
        //change start
        $this->db->where('dipID', trim($this->input->post('dipID')));
        //change end

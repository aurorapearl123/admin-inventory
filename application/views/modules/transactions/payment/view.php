				...
				<div class="card-body">
					<div class="data-view">
						<table class="view-table">
							...
							<!-- Table Rows Start -->
								<tr>
									<td class="data-title" style="width:120px" nowrap>Payment No:</td>
									<td class="data-input" style="width:420px" nowrap><?php echo $rec->paymentNo; ?></td>
									<td class="data-title" style="width:120px" nowrap>Payment Date:</td>
									<td class="data-input" style="width:420px" nowrap><?php echo $rec->paymentDate == '0000-00-00' ? '' :date('M d,Y',strtotime($rec->paymentDate)); ?></td>
                      ...
                      <div class="table-responsive-xl">
                        <table id="" class="table hover">
                          <thead class="thead-light">
                            <tr>
								<th class="text-center w-10" align="center">PO No</th>
								<th class="text-center w-10" align="center">RR No</th>
								<th class="text-center w-10" align="center">Delivery Date</th>
								<th class="text-right w-10" align="right">Payment Amount</th>
								<th class="text-right w-10" align="right">Total Amount Paid</th>
								<th class="text-right w-10" align="right">RR Balance</th>
                            </tr>
                          </thead>
                          <tbody>
                        	<?php 
                        	...
                        	if (!(empty($rec)) && !(empty($details))) {                        	
                        		...
                        		<tr class="rowdetails">
	                        		<td align="center"><?php echo $row->rrNo;?></td>	                        		
	                        		<td align="center"><?php echo $row->poNo;?></td>	                        		
	                        		<td align="center"><?php echo date('M d, Y', strtotime($row->timeDelivered));?></td> 
	                        		<td align="right"><?php echo number_format($row->amount, 2) ?></td>
	                        		<td align="right"><?php echo number_format($row->amountPaid, 2) ?></td>
	                        		<td align="right"><?php echo number_format($row->rrTotalAmount - $row->amountPaid, 2) ?></td>
                        			
                        		</tr>
                        		<?php 
                        			$totalamount += $row->amount;
                        			$totalPaid += $row->amountPaid;
                        			$totalBalance += ($row->rrTotalAmount - $row->amountPaid);
                        		} //end of foreach ?>
								<tr class="bg-light">
...
<script>
	...
	function confirm_record(status='') {
		...
		var id = "<?php echo $rec->paymentID; ?>";
		var apID = "<?php echo $rec->apID; ?>";
		swal({
		...
		.then((willDelete) => {
			if (willDelete.value) {
				...
				$.post("<?php echo $controller_page; ?>/confirm_record", { id: id, status: status, apID: apID },
					function(response){ 

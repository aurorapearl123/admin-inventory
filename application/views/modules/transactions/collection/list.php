								...
								<table align="left" class="table table-striped hover">
									<thead>
										<tr>
											...
											<?php 
												$headers = array(
												    array('column_header'=>'Collection No','column_field'=>'collectionNo','width'=>'w-10','align'=>''),
												    array('column_header'=>'Collection Date','column_field'=>'collectionDate','width'=>'w-10','align'=>''),
												    array('column_header'=>'Invoice No','column_field'=>'invoiceNo','width'=>'w-10','align'=>''),
												    array('column_header'=>'OR No','column_field'=>'orNo','width'=>'w-10','align'=>''),
												    array('column_header'=>'Status','column_field'=>'status','width'=>'w-5 text-center','align'=>'center'),
												);
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<input type="text" class="form-control" id="collectionNo" name="collectionNo"  value="<?php echo $collectionNo ?>">
											</th>
											<th class="form-group form-input">
											...
											if (count($records)) {
											    ...
											    ?>
													...
													<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->$pfield); ?>'">
														<td><span><?php echo $row->collectionNo ?></span></td>
														<td><span><?php echo $row->collectionDate == '0000-00-00' ? '' : date('M d,Y',strtotime($row->collectionDate)); ?></span></td>
														<td><span><?php echo $row->invoiceNo ?></span></td>
														<td><span><?php echo $row->orNo ?></span></td>

...
class Loyalty extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->_init_setup('Master Files', 'Loyalty', 'loyalty', 'loyaltyID', 'loyaltyNo');
		//change end
	}
    ...
    private function submenu()
    {
        ...
        //change start
        $this->data['current_module'] = $this->modules[$this->module]['sub']['Loyalty']; // defines the current sub module
        //change end
    ...
    public function save()
    {
        ...
        //change start
        $table_fields = array('custID','name', 'rankID', 'productID');
        //change end
        ...
        if ($this->roles['create']) {
            ...
            $this->db->select('loyaltyNo');
            $this->db->where('status !=','-100');
            $this->db->order_by('loyaltyID','desc');           
            $loyalty = $this->db->get('loyalty')->row();

            $year = date('y'); 
            
            if (!$loyalty) {
                $loyaltyNo = $year.str_pad(1, 4, "0", STR_PAD_LEFT);               
            } else {
                if ($year != substr($loyalty->loyaltyNo,0,2)) {
                    $loyaltyNo = $year.str_pad(1, 4, "0", STR_PAD_LEFT);
                } else {
                    $loyaltyNo = str_pad($loyalty->loyaltyNo + 1, 4, "0", STR_PAD_LEFT);
                }
            }                
                        
            $this->record->fields['loyaltyNo']       = $loyaltyNo;          
            $this->record->fields['expiry']          = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') .'+1 year'));          
            ...
            if ($this->record->save()) {
                ...
                $logs = "Insert - " .trim($this->input->post('name'));
    ...
    {   
        ...
        if ($this->roles['edit']) {
            ...
            $this->record->fields[] = 'products.name as productName';       
            $this->record->fields[] = 'loyalty_ranks.rankName';       
        
            // set joins
            $this->record->joins[]  = array('customers',$this->table.'.custID=customers.custID','left');
            $this->record->joins[]  = array('products',$this->table.'.productID=products.productID','left');
            $this->record->joins[]  = array('loyalty_ranks',$this->table.'.rankID=loyalty_ranks.rankID','left');
    ...
    public function update()
    {
        ...
        // var_dump($id); die();
        //change start
        $table_fields = array('custID','name', 'productID');
        //change end
    ...
    public function show()
    {
        ...
        $condition_fields = array(            
            array('variable'=>'loyaltyNo', 'field'=>$this->table.'.loyaltyNo', 'default_value'=>'', 'operator'=>'where'),
            ...
            array('variable'=>'rank', 'field'=>$this->table.'.rank', 'default_value'=>'', 'operator'=>'like_both'),           
            array('variable'=>'expiry', 'field'=>$this->table.'.expiry', 'default_value'=>'', 'operator'=>'where'),
        ...
        $this->db->select('loyalty_ranks.rankName');
        $this->db->select('products.name as productName');
        
        // from
        $this->db->from($this->table);
        $this->db->join('customers',$this->table.'.custID=customers.custID','left');
        $this->db->join('loyalty_ranks',$this->table.'.rankID=loyalty_ranks.rankID','left');
        $this->db->join('products',$this->table.'.productID=products.productID','left');
    ...
    public function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->loyaltyNo . "</Data></Cell>";
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->rankName . "</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . date('M Y',strtotime($row->expiry)) . "</Data></Cell>";
                if ($row->status == 1) {

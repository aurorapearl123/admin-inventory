	...
	<div class="row">
		<div class="col-12">
			<div class="card-box">
				...
				<div class="card-body">
					<form method="post" name="frmEntry" id="frmEntry" action="<?php echo site_url('province/update') ?>">
						<input type="hidden" name="provinceID" id="prvinceID" value="<?php echo $this->encrypter->encode($rec->provinceID) ?>" />
						<div class="table-row">
								...
								<tbody>
									<tr>
										...
										<td class="form-group form-input" style="width:400px" nowrap>
											<select class="form-control" id="countryID" name="countryID" data-live-search="true" liveSearchNormalize="true" title="Countries" required>
												<option value="">&nbsp;</option>
												<?php 
												$recs = $this->db->get('countries')->result();
												foreach($recs as $row){
												?>
												<option value="<?php echo $row->countryID?>" <?php if ($row->countryID == $rec->countryID) { echo "selected"; } ?>><?php echo $row->country?></option>
												<?php } ?>
											</select>
										</td>
									...
									<tr>
										...
										<td class="form-group form-input" nowrap><input type="text" class="form-control" name="province" id="province" value="<?php echo $rec->province ?>" title="Province" required></td>
										<td class="d-xxl-none"></td>
									</tr>
									<tr>
...
<script>
	...
	$('#cmdCancel').click(function(){
	    ...
	    .then((willDelete) => {
	      if (willDelete.value) {
	    	  window.location = '<?php echo site_url('province/view/'.$this->encrypter->encode($rec->provinceID)) ?>';
	      }
	    });
	});
</script>

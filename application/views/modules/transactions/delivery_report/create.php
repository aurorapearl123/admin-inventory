			...
			<div class="card-box">
				...
				<div class="card-body">
					<form method="post" name="frmEntry" id="frmEntry" action="<?php echo $controller_page ?>/save" enctype="multipart/form-data">
						<input type="hidden" class="form-control" name="soID2" id="soID2"  >
						<input type="hidden" class="form-control" name="custID2" id="custID2"  >
						<div class="table-row">
							<table class="table-form column-3">
								<tbody class="">
									<tr>
										<td class="form-label" width="12%">DR No:<span class="asterisk">*</span></td>
										<td class="form-group form-input" width="21.33%">
											<input type="text" class="form-control" id="drNo" name="drNo" title="DR No" value="<?php echo $series ?>" readonly required>
										</td>
										<td class="form-label" width="12%">DR Date: <span class="asterisk">*</span></td>
										<td class="form-group form-input" width="21.33%">
									...
									<tr>
										<td class="form-label">Driver Name: </td>
										<td class="form-group form-input">
											<input type="text" class="form-control" id="driverName" name="driverName" title="Driver Name">
										</td>
										<td class="form-label">Plate No: </td>
										<td class="form-group form-input">
											<input type="text" class="form-control" id="plateNo" name="plateNo" title="Plate No">
										</td>
									...
									<tr>
										...
										<td class="form-group form-input">
											<input type="text" class="form-control datepicker" id="departure" name="departure" data-toggle="datetimepicker" value="" data-target="#departure" title="Delivery Date">
										</td>
										<td class="form-label"></td>
										<td class="form-group form-input">
					...
					<div class="datatables_wrapper">
						<div class="table-responsive-md">
							<form method="post" name="frmAdd" id="frmAdd" ><!-- Form Start -->
								...
								<input type="hidden" name="sessionSet" id="sessionSet" value="drdetails" />						
								<table id="" class="table">
									<thead class="thead-light" align="center">
										<tr>
											<th class="dataField w-15" nowrap>Customer</th>
											<th class="dataField w-15" nowrap>SO No</th>
										...
										<tr>
											<th>
												<select id="custID" name="custID" class="form-control" data-live-search="true" liveSearchNormalize="true" title="Customer" onchange="get_so_customers();" required>
													<option value="">&nbsp;</option>
													<?php 
													$this->db->where('status >', 0);
													$results = $this->db->get('customers')->result();
													foreach($results as $res){
														?>
														<option value="<?php echo $res->custID?>" ><?php echo $res->companyName; ?></option>
													<?php }?>
												</select>
											</th>
											<th>
												<select id="soID" name="soID" class="form-control" data-live-search="true" liveSearchNormalize="true" title="Sales Order" onchange="get_so_select_products();" required>
													<option value="">&nbsp;</option>
											...
											<th class="form-group form-input w-10" align="left" >
												...
												<select class="form-control country" id="productID" name="productID" data-live-search="true" livesearchnormalize="true" title="product"  onchange="set_sodetails_product();" required>
...
if (page_type == 'view') {
	tm_display_session_products('drdetails', 'div_products');
}
...
//change end
echo $this->htmlhelper->get_json_select('get_so_customers', $parameters, site_url('generic_ajax/get_so_customers'), 'soID', '', '') ;

//change start
$parameters = array('soID');
//change end
echo $this->htmlhelper->get_json_select('get_so_select_products', $parameters, site_url('generic_ajax/get_so_select_products'), 'productID', '', 'confirm_change();') ;
?>

function confirm_change()
{
	if ($('#soID2').val() != '' && $('#soID2').val() != $('#soID').val()) {
		swal({
		...
		.then((willDelete) => {
			if (willDelete.value) {
				...
				//change start
				$('#soID').selectpicker('val', $('#soID2').val());
...
function set_sodetails_product()
{
	//change start
	//details
	...
	var soID = $('#soID').val();
	//path
	var path = "generic_ajax/set_sodetails_product/";
	//change end

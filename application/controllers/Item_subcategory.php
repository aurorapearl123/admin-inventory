...
class Item_subcategory extends CI_Controller {
	//Default Variables
	var $menu;
	var $roles;
	var $data;
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('item_subcategory'); // defines contoller link
		$this->table = 'subcategory'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'subcatID'; // defines primary key
		$this->logfield = 'subcatID';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Item_Subcategory'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Item Subcategory']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('catID','subcatdesc');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['subcatID'] = $this->db->insert_id();
				$this->records->retrieve();
				// record logs
				$logs = "Record - ".$id;
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('catID','subcatdesc', 'status');
		...
		if ($this->roles ['delete']) {
			...
			if (! empty ( $this->records->field )) {
				...
				if (! $this->_in_used ( $id )) {
					...
					$_data = [
						'deletedBy' =>  $this->session_id,
						'status' =>  $this->deleted_status,
						'dateDeleted' => $this->date_today
					];
					$is_updated = $this->db->update($this->table, $_data);
					if ($is_updated) {
						// record logs
						$logfield = $this->logfield;						
						$logs = "Record - " . $this->encrypter->decode($this->records->field->$logfield);
	...
	public function show() {
		...
		$condition_fields = array (
			...
			array ('variable' => 'subcatdesc', 'field' => $this->table . '.subcatdesc', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) 
		);
		
		// sorting fields
		$sorting_fields = array ('subcatID' => 'asc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->subcatdesc."</Data></Cell>";
				if ($row->status == 1) {
	...
	public function check_duplicate() {
		...
		$subcatdesc = trim($this->input->post('subcatdesc'));
		$this->db->where('catID', $catID);
		$this->db->where('subcatdesc', $subcatdesc);
		$this->db->where_not_in('status', $this->deleted_status);

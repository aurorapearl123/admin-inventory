...
class Purchase_order extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->_init_setup('Transactions', 'purchase_order', 'poheaders', 'poID', 'poNo');
		//change end
	}
    ...
    private function submenu()
    {
        ...
        //change start
        $this->data['current_module'] = $this->modules[$this->module]['sub']['Purchase Order']; // defines the current sub module
        //change end
    ...
    public function create()
    {
        ...
        if ($this->roles['create']) {
            ...
            $this->frameworkhelper->clear_session_item('podetails');
    ...
    public function save()
    {
        ...
        //change start
        $table_fields = array('poNo','suppID','discount','paymentTerm','amountPaid','balance','remarks','deliveryTerm', 'discountType');
        //change end
        

        // poID    poNo    poDate  suppID  grossAmount discount    netAmount   paymentTerm    dueDate amountPaid  balance datePaid    createdBy   dateCreated confirmedBy dateConfirmed   cancelledBy dateCancelled   dateInserted    dateLastEdit    dateDeleted deletedBy   remarks status 1: pending; 2: confirm; 3: partial delivery; 4: full
        ...
        if ($this->roles['create']) {
            ...
            if ($this->record->save()) {
                ...
                // id  poID    productID   qty price   amount  delQty  delStatus   dateInserted    dateLastEdit    dateDeleted deletedBy   status 1: pending; 2: confirm; 3: partial delivery; 4: full
                $total = 0;
                if (!empty($_SESSION['podetails'])) {
                    $batch = array();
                    foreach($_SESSION['podetails'] as $product){
                        $info = array();
                        $info['poID']           = $this->record->field->poID;
                    ...
                    $this->db->insert_batch('podetails', $batch);
                }
                //change end


                //send notification
                $message = 'New Purchase Order Created - '.$this->record->field->poNo;
                $this->record->send_notification($message, $this->record->field->poID);
    ...
    {   
        ...
        if ($this->roles['edit']) {
            ...
            // ----------------------------------------------------------------------------------
            $this->db->select('podetails.poID');
            $this->db->select('podetails.qty as productQty');
            $this->db->select('podetails.price as productPrice');
            $this->db->select('podetails.amount as productAmount');  
            $this->db->select('podetails.productID');        
            ...
            $this->db->where('podetails.poID', $data['rec']->poID);
    ...
    public function update()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $id = $this->encrypter->decode($this->input->post('poID'));
        //change start
        $table_fields = array('poNo','suppID','discount','paymentTerm','amountPaid','balance','remarks','deliveryTerm','discountType');
        //change end
        // check roles
    ...
    public function view_decoded($id, $notificationID)
    {
        ...
        if ($this->roles['view']) {
            ...
            $this->db->select('podetails.price');
            $this->db->select('podetails.amount');  
            $this->db->select('podetails.productID');        
            ...
            $this->db->where('podetails.poID', $data['rec']->poID);
            $this->db->where('podetails.status != -100');
    ...
    public function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                $poDate = $row->poDate == '0000-00-00' ? '' : date("M d, Y",strtotime($row->poDate));
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $poDate . "</Data></Cell>";
    ...
    {
        //change start
        $this->db->where('poNo', trim($this->input->post('poNo')));
        //change end
    ...
    public function print_record($id=0)
    {
        ...
        // for retrieve with joining tables -------------------------------------------------
            ...
            $this->db->where($this->pfield, $id);
            // ----------------------------------------------------------------------------------
            $data['poheaders'] = $this->db->get()->row();
            ...
            $this->db->where('podetails.poID', $data['poheaders']->poID);
            $this->db->where('podetails.status !=',-100);

            $this->db->order_by('products.productID','ASC');
            $data['podetails'] = $this->db->get()->result();
            ...
            $this->mpdf->Output("PURCHASE_ORDER_".$data['poheaders']->poNo.".pdf","I");

...
class Pump extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('pump'); // defines contoller link
		$this->table = 'pumps'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'pumpID'; // defines primary key
		$this->logfield = 'pumpName';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Pump'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Pump']; // defines the current sub module
		// check roles
		$this->check_roles ();
...
//	}
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('pumpCode','pumpName');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['pumpID'] = $this->db->insert_id();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('pumpCode','pumpName','status');
	...
	public function show() {
		...
		$condition_fields = array (
			array ('variable' => 'pumpCode', 'field' => $this->table . '.pumpCode', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'pumpName', 'field' => $this->table . '.pumpName', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('pumpName' => 'asc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->pumpCode."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->pumpName."</Data></Cell>";
				if ($row->status == 1) {
	...
	public function check_duplicate() {
		$this->db->where('pumpName', trim($this->input->post('pumpName')));

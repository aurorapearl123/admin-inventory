					...
					<div class="card-head">
						...
						<div class="card-head-tools">
							<ul class="tools-list">
								...
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Print" onclick="popUp('<?php echo site_url('dipstick/printlist') ?>', 800, 500)"><i class="la la-print"></i></button>
								</li>
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Export to Excel File" onclick="window.location='<?php echo site_url('dipstick/exportlist') ?>';"><i class="la la-file-excel-o"></i></button>
								</li>
							</ul>
						</div>
					</div>
									...
									<thead>
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<?php  $inTime = ($inTime != NULL) ? strtotime($inTime) : "--:-- --"?>
												<input type="time" id="inTime" class="form-control" name="inTime"  value="<?php echo date('h:i:s', $inTime )?>" >
											</th>

											<th class="form-group form-input">
												<?php  $outTime = ($outTime != NULL) ? strtotime($outTime) : "--:-- --"?>
												<input type="time" id="outTime" class="form-control" name="outTime" value="<?php echo date('h:i:s', $outTime)?>" >
											</th>
											<th class="form-group form-input">
									...
									<tbody>
										<?php 
											if (count($records)) {
											    ...
											    ?>
										<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->dipID); ?>'">
											<td><?php echo date('G:i A', strtotime($row->inTime)) ?></td>
											<td><?php echo date('G:i A', strtotime($row->outTime)) ?></td>
											<td><?php echo $row->openingQty ?></td>
											<td><?php echo $row->closingQty ?></td>
											<td><?php echo $row->varianceQty ?></td>
											<td><?php echo $row->openingMsr ?></td>
											<td><?php echo $row->closingMsr ?></td>
											<td><?php echo $row->varianceQty ?></td>
											<td align="center">
												<?php 
												if ($row->status == 1) {

...
class Item_category extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('item_category'); // defines contoller link
		$this->table = 'category'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'catID'; // defines primary key
		$this->logfield = 'categoru';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Item_Category'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Item Category']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('category', 'description');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['catID'] = $this->db->insert_id();
				$this->records->retrieve();
				// record logs
				$logs = "Record - ".trim($this->input->post($this->logfield));
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('category','description', 'status');
	...
	public function show() {
		...
		$condition_fields = array (
			array ('variable' => 'category', 'field' => $this->table . '.category', 'default_value' => '', 'operator' => 'like_both' ),
		...
		// sorting fields
		$sorting_fields = array ('catID' => 'asc' );

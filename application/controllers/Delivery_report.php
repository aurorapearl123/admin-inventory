...
class Delivery_report extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->_init_setup('Transactions', 'delivery_report', 'drheaders', 'drID', 'drNo');
        //change end
    }
    ...
    private function submenu()
    {
        ...
        //change start
        $this->data['current_module'] = $this->modules[$this->module]['sub']['Delivery Report']; // defines the current sub module
        //change end
    ...
    public function create()
    {
        ...
        if ($this->roles['create']) {
            ...
            $this->frameworkhelper->clear_session_item('drdetails');
    ...
    public function save()
    {
        ...
        //change start
        $table_fields = array('drNo', 'plateNo', 'driverName', 'driverAssistant');
        //change end
        
        

        // drID    drNo    poDate  custID  grossAmount discount    netAmount   paymentTerm    dueDate amountPaid  balance datePaid    createdBy   dateCreated confirmedBy dateConfirmed   cancelledBy dateCancelled   dateInserted    dateLastEdit    dateDeleted deletedBy   remarks status 1: pending; 2: confirm; 3: partial delivery; 4: full
        ...
        if ($this->roles['create']) {
            ...
            //change start
            $this->record->fields['soID']           = trim($this->input->post('soID2'));
            $this->record->fields['custID']           = trim($this->input->post('custID2'));
            $this->record->fields['drDate']        = date('Y-m-d', strtotime(trim($this->input->post('drDate'))));
            $this->record->fields['departure']    = date('Y-m-d', strtotime(trim($this->input->post('departure'))));
            ...
            if ($this->record->save()) {
                ...
                // id  drID    productID   qty price   amount  delQty  delStatus   dateInserted    dateLastEdit    dateDeleted deletedBy   status 1: pending; 2: confirm; 3: partial delivery; 4: full
                $total = 0;
                if (!empty($_SESSION['drdetails'])) {
                    $batch = array();
                    foreach($_SESSION['drdetails'] as $product){
                        $info = array();
                        $info['drID']           = $this->record->field->drID;
                    ...
                    $this->db->insert_batch('drdetails', $batch);
                }
                //change end
    ...
    {   
        ...
        if ($this->roles['edit']) {
            ...
            // ----------------------------------------------------------------------------------
            $this->db->select('drdetails.drID');
            $this->db->select('drdetails.qty as productQty');
            $this->db->select('drdetails.price as productPrice');
            $this->db->select('drdetails.amount as productAmount');  
            $this->db->select('drdetails.productID');        
            ...
            $this->db->where('drdetails.drID', $data['rec']->drID);
            $this->db->where('drdetails.status >', 0);
            $drdetails = $this->db->get()->result_array();
            // productID,productUmsr,productName,productQty,productPrice,productAmount,productBrand
            

            //Details
            //change start
            $sessionSet = 'drdetails';
            $this->frameworkhelper->clear_session_item($sessionSet);


            if (!empty($drdetails)) {
                foreach ($drdetails as $new_item) {
    ...
    public function update()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $id = $this->encrypter->decode($this->input->post('drID'));
        //change start
        $table_fields = array('drNo', 'plateNo', 'driverName', 'driverAssistant');
        //change end
    ...
    public function view($id)
    {
        ...
        if ($this->roles['view']) {
            ...
            // extend fields - join tables          
            //change start
            // $this->record->fields[]  = 'drdetails.*';
            // $this->record->fields[] = 'products.*';
            $this->record->fields[] = 'customers.custID';
            $this->record->fields[] = 'customers.companyName';
            $this->record->fields[] = 'customers.streetNo';
            $this->record->fields[] = 'soheaders.soNo';
            $this->record->fields[] = 'soheaders.soID';

            
            // set joins                   
            $this->record->joins[]  = array('customers',$this->table.'.custID=customers.custID','left');  
            $this->record->joins[]  = array('soheaders',$this->table.'.soID=soheaders.soID','left');  
            ...
            //change end
            $this->db->select('drdetails.drID');
            $this->db->select('drdetails.qty');
            // $this->db->select('drdetails.delQty');
            $this->db->select('drdetails.price');
            $this->db->select('drdetails.amount');  
            $this->db->select('drdetails.productID');        
    ...
    public function show()
    {
        ...
        $condition_fields = array(
            ...
            array('variable'=>'drDate', 'field'=>$this->table.'.drDate', 'default_value'=>'', 'operator'=>'where'),
            
            array('variable'=>'companyName', 'field'=>'customers.companyName', 'default_value'=>'', 'operator'=>'like_both'),
            array('variable'=>'soNo', 'field'=>$this->table.'.soNo', 'default_value'=>'', 'operator'=>'like_both'),
            ...
            array('variable'=>'departure', 'field'=>$this->table.'.departure', 'default_value'=>'', 'operator'=>'like_both'),
            array('variable'=>'status', 'field'=>$this->table.'.status', 'default_value'=>'', 'operator'=>'where'),
        );
        
        // sorting fields
        $sorting_fields = array('drNo'=>'desc','drDate'=>'desc');
        //change end
        //=========================
        ...
        $this->db->join('soheaders',$this->table.'.soID=soheaders.soID','left');
    ...
    public function printlist()
    {
        ...
        // sorting fields
        $sorting_fields = array('drNo'=>'asc','poDate'=>'asc');
        //change end
        //=========================
    ...
    public function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                $drDate = $row->drDate == '0000-00-00' ? '' : date("M d, Y",strtotime($row->drDate));
                $departure = $row->departure == '0000-00-00' ? '' : date("M d, Y",strtotime($row->departure));
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $drDate . "</Data></Cell>";
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $departure . "</Data></Cell>";
                if ($row->status == 1) {
    ...
    {
        //change start
        $this->db->where('drNo', trim($this->input->post('drNo')));
        //change end
    ...
    public function confirm_record()
    {
        ...
        if ($status == 2) {
            ...
            $records = $this->confirm_dr($id, $soID);
            
            if ($records) {
                foreach ($records as $row) {

                    $message = 'SO No. '.$row->poNo.' - '.'Fully Delivered.';

                    $qtyValue = $row->newQty;
                    $refNo = 'SO No.'.$row->poNo.'/DR No.'.$row->drNo;
                    $this->update_stockcards('CR', $row->productID, $qtyValue, $refNo, $row->currentPrice, $row->lastcost, $row->markupType, $row->markup);

                    $this->update_customer_ledger('DR', $row->productID, $qtyValue, $refNo, $custID);
                }
            }
            ...
            $this->db->select('drdetails.qty');
            $this->db->select('drdetails.productID');
            $this->db->select('drheaders.soID');
            $this->db->from('drdetails');
            $this->db->join('drheaders', 'drdetails.drID=drheaders.drID', 'left');
            $this->db->join('soheaders', 'drheaders.soID=soheaders.soID', 'left');
            $this->db->where('soheaders.soID', $soID);
            $this->db->where('drheaders.drID', $id);
            // $this->db->where('soheaders.status >', 0);
            $records = $this->db->get()->result();

            if (!empty($records)) {
                foreach ($records as $row) {
                    $this->db->set('sodetails.delQty', 'delQty+'.$row->qty, false);
                    $this->db->where('sodetails.productID', $row->productID);
                    $this->db->where('sodetails.soID', $row->soID);
            ...
            if (!empty($records)) {
                foreach ($records as $row) {
                    if ($row->delQty >= $row->qty) {
                        $this->db->set('sodetails.status', 2);
                        $this->db->where('sodetails.id', $row->id);
                        $this->db->update('sodetails');
                    }
                }
            }
            ...
            if ($count == 0) {
                ...
                $this->record->send_notification($message, $soID);
            } else {
    ...
    private function update_customer_ledger($type='', $productID=0, $qtyValue=0, $refNo='', $custID)
    {
        $this->db->select('customer_ledger.id');
        $this->db->select('customer_ledger.productID');
        $this->db->select('customer_ledger.endBal');
        $this->db->select('products.name as productName');
        $this->db->select('products.description as productDescription');
        $this->db->select('products.umsr as productUmsr');
        $this->db->from('customer_ledger');  
        $this->db->join('products', 'customer_ledger.productID=products.productID', 'left');
        $this->db->where('customer_ledger.productID', $productID);
        $this->db->where('customer_ledger.custID', $custID);
        $product = $this->db->get()->last_row();
        ...
        if (!empty($product)) {
            ...
            $this->db->set('refNo', $refNo);
            $this->db->set('dateInserted', date('Y-m-d H:i:s'));
            $this->db->insert('customer_ledger');
    ...
    private function confirm_dr($drID, $soID)
    {
        
        $this->db->set('status', 2);        
        $this->db->where('drID', $drID);        
        $this->db->update('drheaders');

        $this->db->set('status', 2);        
        $this->db->where('drID', $drID);        
        $this->db->update('drdetails');
    
        $records = $this->get_dr_details($drID);
    ...
    private function get_dr_details($drID)
    {
        $this->db->select('drdetails.id as drDetailID');
        $this->db->select('drdetails.drID');
        $this->db->select('drdetails.productID');
        $this->db->select('drdetails.price as currentPrice');
        $this->db->select('drdetails.qty as newQty');
        ...
        $this->db->join('drheaders', 'drdetails.drID=drheaders.drID', 'left');
        $this->db->join('soheaders', 'drheaders.soID=soheaders.soID', 'left');
        $this->db->where('drdetails.drID', $drID);    
        $this->db->where('drheaders.drID', $drID);        
        $this->db->where('drheaders.status >', 0);      
        $this->db->where('drdetails.status >', 0);
        $records = $this->db->get()->result();
    ...
    public function print_record($id=0)
    {
        ...
        // for retrieve with joining tables -------------------------------------------------
            ...
            $this->db->where($this->pfield, $id);
            // ----------------------------------------------------------------------------------
            $data['drheaders'] = $this->db->get()->row();
            ...
            $this->db->where('drdetails.drID', $data['drheaders']->drID);
            $this->db->where('drdetails.status !=',-100);

            $this->db->order_by('products.productID','ASC');
            $data['drdetails'] = $this->db->get()->result();

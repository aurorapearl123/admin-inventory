...
class Signatory extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('signatory'); // defines contoller link
		$this->table = 'signatories'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'signatoryID'; // defines primary key
		$this->logfield = 'fullname';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/Signatory'; // defines module path
	...
	private function submenu() {
		...
		$this->data['current_module'] = $this->modules[$this->module]['sub']['Signatory']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu();
		$data = $this->data;
		$table_fields = array ('code','fullname', 'designation1', 'designation2', 'designation3');
		...
		if ($this->roles['create']) {
			...
			if ($this->records->save()) {
				$this->records->fields = array();
				$id = $this->records->where['signatoryID'] = $this->db->insert_id();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('code','fullname', 'designation1', 'designation2', 'designation3','status');
	...
	public function show() {
		...
		$condition_fields = array (
			array ('variable' => 'fullname', 'field' => $this->table . '.fullname', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'designation1', 'field' => $this->table . '.designation1', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'designation2', 'field' => $this->table . '.designation2', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'designation3', 'field' => $this->table . '.designation3', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('fullname' => 'asc' );
	...
	function exportlist() {
		...
		$fields[] = "DESIGNATION 2";
		$fields[] = "DESIGNATION 3";
		$fields[] = "STATUS";
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->fullname."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->designation1."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->designation2."</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->designation3."</Data></Cell>";
				if ($row->status == 1) {
	...
	public function check_duplicate() {
		...
		$this->db->where('fullname', trim($this->input->post('fullname')));

...
class City extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data ['controller_page'] = $this->controller_page = site_url ( 'city' ); // defines contoller link
		$this->table = 'cities'; // defines the default table
		$this->pfield = $this->data ['pfield'] = 'cityID'; // defines primary key
		$this->logfield = 'city';
		$this->module_path = 'modules/' . strtolower ( str_replace ( " ", "_", $this->module ) ) . '/city'; // defines module path
	...
	private function submenu() {
		...
		$this->data ['current_module'] = $this->modules [$this->module] ['sub'] ['City']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('provinceID','city', 'zipcode', 'remarks');
		...
		if ($this->roles ['create']) {
			...
			if ($this->records->save ()) {
				$this->records->fields = array ();
				$id = $this->records->where ['cityID'] = $this->db->insert_id ();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('provinceID','city', 'zipcode', 'remarks', 'status');
	...
	public function show() {
		...
		$condition_fields = array (
			array ('variable' => 'countryID', 'field' => $this->table . '.countryID', 'default_value' => '', 'operator' => 'where' ), 
			array ('variable' => 'provinceID', 'field' => $this->table . '.provinceID', 'default_value' => '', 'operator' => 'where' ), 
			array ('variable' => 'city', 'field' => $this->table . '.city', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'zipcode', 'field' => $this->table.'.zipcode', 'default_value' => '', 'operator' => 'like_both' ), 
			array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('city' => 'asc','status' => 'desc' );
		...
		// join
		$this->db->join ( 'countries', $this->table . '.countryID=countries.countryID', 'left' );
		$this->db->join ( 'provinces', $this->table . '.provinceID=provinces.provinceID', 'left' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->zipcode . "</Data></Cell>";
				if ($row->status == 1) {
					$data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Active</Data></Cell>";
				} else {
					$data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Dissolved</Data></Cell>";
				}
				$data .= "</Row>";
	...
	public function check_duplicate() {
		$this->db->where('city', trim($this->input->post('city')));
